...
CREATE TABLE IF NOT EXISTS public.app_users (
    ...
    CONSTRAINT app_users_username_not_empty CHECK (length(trim(coalesce(username, ''))) > 0),
    CONSTRAINT app_users_password_not_empty CHECK (length(trim(coalesce(password_hash, ''))) > 0)
);

CREATE UNIQUE INDEX IF NOT EXISTS app_users_username_ci_idx ON public.app_users (lower(username));
