...
const MAX_WITH_CLIENT_ATTEMPTS = 3;
...
export async function withClient<T>(fn: (c: PoolClient) => Promise<T>): Promise<T> {
  ...
  while (attempt < MAX_WITH_CLIENT_ATTEMPTS) {
    ...
    try {
      ...
      const isRetryable = shouldResetPoolForError(err);
      ...
      logger.warn({ err, attempt, delayMs }, 'PG connect failed; resetting pool and retrying after backoff');
      await resetPool();
