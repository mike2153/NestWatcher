...
export async function computeOrderingRows(): Promise<OrderingComputation> {
  ...
  const [prereservedRows, lockedRows] = await Promise.all([
    withClient((c) =>
      c
        .query<PendingJobRow>(
          `
            ...
            FROM public.jobs
            WHERE pre_reserved = TRUE AND status = 'PENDING'
            GROUP BY material
          `
        )
        .then((res) => res.rows)
  ...
  const prereservedByKey = new Map<string, number>();
  for (const row of prereservedRows) {
    prereservedByKey.set(normalizeMaterialKey(row.material), row.required_count);
  }
  ...
  // Combine all materials from both prereserved and locked
  const allMaterialKeys = new Set<string>([
    ...Array.from(prereservedByKey.keys()),
  ...
  const aggregated = Array.from(allMaterialKeys).map((materialKey) => {
    const prereservedCount = prereservedByKey.get(materialKey) ?? 0;
    const lockedCount = lockedByKey.get(materialKey) ?? 0;
    // Get the raw material string from one of the original rows
    const prereservedRow = prereservedRows.find(r => normalizeMaterialKey(r.material) === materialKey);
    const lockedRow = lockedRows.find(r => normalizeMaterialKey(r.material) === materialKey);
    const material = (prereservedRow?.material ?? lockedRow?.material ?? '').trim();

    return {
      material,
      materialKey,
      prereservedCount: Math.max(prereservedCount, 0),
  ...
  for (const entry of aggregated) {
    ...
    // New calculation: shortage = (prereserved + locked) - stock
    const prereservedCount = entry.prereservedCount;
    const lockedCount = entry.lockedCount;
    const totalRequired = prereservedCount + lockedCount;
    const currentStock = numericZeroed(stock);

    const orderAmount = totalRequired - currentStock;
    ...
    const effectiveAvailable = currentStock;
  ...
  if (!items.some((item) => item.materialKey === UNKNOWN_KEY)) {
    ...
    if (unknownPending) {
      const totalRequired = unknownPending.prereservedCount + unknownPending.lockedCount;
      if (totalRequired > 0) {
        items.push({
          id: null,
          typeData: null,
...
export async function updateOrderingStatus(
  ...
  return withClient(async (client) => {
    ...
    // In case order no longer appears (e.g., order amount resolved), fabricate a placeholder row.
    return {
      id,
      typeData: null,
      customerId: null,
      materialKey: String(id),
      materialLabel: String(id),
      required: 0,
      lockedCount: 0,
      stock: null,
      stockAvailable: null,
