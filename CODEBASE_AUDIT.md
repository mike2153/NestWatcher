...
- Evidence: Main stores `ThemePreference = 'system' | 'light' | 'dark' | 'modern'` and applies it to Electron (`packages/main/src/services/uiState.ts:7-133`). The renderer’s `ThemeContext` only knows `'dark' | 'sunset' | 'forest' | 'supabase' | 'nccat-light'` and persists to `localStorage` (`packages/renderer/src/contexts/ThemeContext.tsx:1-70`).  
...
4) Primary/Destructive buttons bypass theme tokens (Low)  
- Evidence: `packages/renderer/src/components/ui/button.tsx:12-27` hard-code `bg-blue-600` and `bg-red-600` for `default`/`destructive` variants, while other variants use tokenized colors.  
- Impact: when you switch to `nccat-light` (or any future palette), primary/destructive buttons stay the same dark blues/reds, which can clash with light surfaces.  
- Fix: map those variants to `--primary` / `--destructive` (and their foregrounds) so they inherit per-theme colors.
...
- Define or remove the missing color/shadow/transition tokens so Tailwind utilities resolve to real values across all themes. Start with the `nccat-light` palette to stabilize light mode.  
